...
export default function CarDetailClient({
  ...
  }, [id, locale, initialCar]);

  useEffect(() => {
    const brandId = car?.brandId ? String(car.brandId).trim() : "";
    if (!car?.id || !brandId) {
      setSimilar([]);
      return;
    }
  ...
  function onCompare() {
    ...
    const result = addCompare(toCompareCar(car));
    if (!result.ok) {
      emitToast(
        result.reason === "full"
  ...
  function onMessageSeller() {
    if (!requireAuth(t(locale, "messageSeller").toLowerCase())) return;
    setRevealBid(true);
  ...
  async function placeBid() {
    ...
    setMessage(null);
    try {
      await api.post(`/cars/${id}/bids`, { amount });
      setMessageTone("ok");
      setMessage(t(locale, "bidSubmitted"));
      setBid("");
      const refreshed = await api.get<Car>(`/cars/${id}`);
      setCar(refreshed);
    } catch (e) {
      setMessageTone("err");
  ...
  if (notFound && !car) {
    return (
      <div className="mx-auto max-w-4xl px-[4%] pt-28 text-center">
        ...
        <div className="mt-6">
          <BackButton label={t(locale, "backToBrowse")} />
        </div>
      </div>
    );
  }
        ...
        <div className="space-y-4">
          ...
          <ListingSellerCard
            ...
            listingSeller={{
              ...
              phone:
                stringField(car, "sellerPhone", "phone", "phoneNumber") || null,
              photoUrl: stringField(car, "sellerAvatar") || null,
            }}
            onMessageSeller={sold ? undefined : onMessageSeller}
          />
          ...
          {showBid ? (
            <div
              id="place-bid"
              className="rounded-[16px] bg-card p-4 ring-1 ring-outline"
            >
              <p className="text-sm font-semibold">{t(locale, "placeBid")}</p>
              {highestAmount != null ? (
                <p className="mt-1 text-xs text-muted" dir="auto">
                  {t(locale, "highestBid", {
                    amount: formatMoney(highestAmount, car.currencyKey),
                  })}
                </p>
              ) : null}
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            ...
            <Spec
              ...
              value={
                localizeOption(
                  locale,
                  stringField(car, "bodyTypeKey", "bodyType"),
                ) || null
              }
            />
            <Spec
            ...
            <Spec
              ...
              value={
                localizeOption(locale, stringField(car, "engineSizeKey")) ||
                null
              }
            />
            <Spec
